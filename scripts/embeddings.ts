...
// ----------------- FONCTIONS ----------------- //
export async function embeddingApi(
  texts: string[],
  dimensions: number,
  save = false
) {
  ...
  if ("error" in brutResults) {
    throw Error("Generation d'embedding ratée :" + brutResults.error.message);
  } else {
    // Sauvegarde des resultats
    const results = brutResults.data.map((data) => data.embedding);

    if (save === true) {
      const currentEmbeddings = JSON.parse(fs.readFileSync(FILE_NAME, "utf8"));
      const newEmbeddings = JSON.stringify([...currentEmbeddings, ...results]);
      fs.writeFile(FILE_NAME, newEmbeddings, (err) => {});
    }
...
async function test() {
  ...
  const embeddings = await embeddingApi(texts, 100, true);
